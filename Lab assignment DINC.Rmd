...
x=nrow(dn)
y=ncol(dn)
"x means the total number of denny's locations"
"the variables are address, city, state, zip, longitude and latitude"
```
...
x2=nrow(lq)
y2=ncol(lq)
"x means the total number of La Quinta locations"
"the variables are address, city, state, zip, longitude and latitude"
```
...
Unsure on La Quinta as filtering by hand is near impossible and there was none outside of the us for Dennys
...
"One way could be to fliter by the intial street number? another could be by state, also could format to contain only certain longitude and laitude that is inside the us."
...
```{r}
dn= mutate(dn, country = "United States")
```
