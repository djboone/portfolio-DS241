...
```{r}
dfall=read.csv("FastFoodRestaurants.csv")
```
...
analyze_state_r = function (this_state) {
  ...
  s=dn_state <- dn %>%
  filter(state == this_state) %>% nrow()
  
  rest_state=dfall %>%
    filter(province==this_state) %>%
...
analyze_state_Q = function (this_state) {
  ...
  dn_lq_state_mindist <- full_join(dn_state, rest_state, by = "state") %>%
    mutate(distance=
           haversine(longitude.x, latitude.x, longitude.y, latitude.y, round = 4)) %>%
  group_by(address.x) %>%
  summarise(closest = min(distance))  
...
```{r}
dftx_LQ = analyze_state_Q("NY")

dftx_LQ %>% ggplot(aes(x=closest)) +geom_histogram()
summary(dftx_LQ)
```
