...
```
```{r}
df_pops=wb_data("SP.POP.TOTL", start_date = 2019, end_date = 2019)
```



```{r}
Corona_TP=left_join(coronavirus_df, df_pops , by=("country")) %>%
mutate(Corona_TP,CBP = cases/SP.POP.TOTL)

```
 
```{r}
ggplot(Corona_TP) + geom_point(aes(x= days_elapsed, y=CBP, color=country))
```
```{r}
...
```
```{r}
ggplot(data = Corona_TP,
       mapping = aes(x = date.x, 
                     y = CBP, 
                     color = country, 
                     )) +
  # represent cumulative cases with lines
